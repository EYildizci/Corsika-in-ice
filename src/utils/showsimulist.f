...
c          Primary, lg(E), theta, phi, nsh, runtsk, size,
...
c-----------------------------------------------------------------------
c compilation:
c           gfortran -fbounds-check showsimulist.f -o showsimulist
c           f77 -fbounds-check showsimulist.f -o showsimulist
c           ifort -C -check bounds showsimulist.f -o showsimulist 
c execution:
c           ls -l DAT?????? | ./showsimulist
c           ls -l [C,D]???????? | ./showsimulist > showsimulist.corsika
c-----------------------------------------------------------------------
c     output-files:
...
c-----------------------------------------------------------------------
c         Primary   lg(E)  theta    phi  nsh runtsk    size  obslvme  h1stme   .....
...
c=======================================================================
      ...
      character qpatext(1:200)*19,chemical(0:100)*12,chfmti*5,chaspec*1

      dimension fsize(nfmx),nfsho(nfmx),ntskf(nfmx),nbits(nfmx)
      dimension pdata(5733),qrunh(312),qevth(312),nflen(nfmx)
...
c--print title lines----------------------------------------------------
     ...
     +   2x,''runtsk    size  obslvme  h1stme  thilev  wmax  thirad'',
...
c--work on all particle data files--------------------------------------
  ...
  422 continue
      ! if ( ifil .ge. 1 ) then
      !   write(*,*) '     64_bit processor reads 32_bit particle data.'
      ! endif
  424 continue
...
c - - - - - check on Aires special primary particle feature - - - - -
c           or other qgsjet Aires simulation by `joe`:
c           ! if ( ntask .ge.  20000 .and. ntask .le. 20999 .or.
      chaspec = ' '
      if ( ntask .ge. 200000 .and. ntask .lt. 209999 ) then
         if ( qrunh(15) .gt. 0. .and. qrunh(5) .lt. 10 )
     +      nfsho(ifil) = qrunh(15) ! number of showers Aires simul. 
         if ( qevth(160) .gt. 0 ) chaspec = '*'
         if ( qevth(79) .eq. 1 ) then
            qevth(73) = 0.
            qevth(74) = 0.
         endif
         naires = naires + 1 
...
c - - - - - check on 64bit simulation and Cerenkov output - - - - -
...
c - - - - - print out tabular quantities - - - - -
     ...
     +    obslev .ne. obslprev .or. lthi   .ne. lthiprev ) then
        ...
        if ( qevth(159) .lt. 1.e-3 ) then
          if ( qevth(44) .lt. 80000. .or. 99900. .lt. qevth(44) ) then
...
**   +,i3,3f7.0)') int(qevth(79)),qevth(160),qevth(161),qevth(162)
        else ! additional Aires weight factor printed.
         if ( fsize(ifil) .lt. 10000. ) then
