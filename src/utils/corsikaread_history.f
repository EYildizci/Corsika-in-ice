...
C----------------------------------------------------------------------
C     compilation:
C      gfortran -fbounds-check corsikaread_history.f \
...
C  read shower header
      ...
      write(6,11)
 11   format(1h ,' ID |  ENERGY  |   P_x   |   P_y    |   P_z    |',
     &           '      x     |      y     |      z',/,
     &  1h ,47x,'|gen.counter |    chi     |      z')
      if (iret.eq.1) then
...
*KEND.
      ...
      double precision xm,Elab,px,py,pz,xx,yy,hh,ehis(1000)
      integer          i,ID_par,Ishower,Nmax
...
C  print muon/mother/grandmother entries
        ...
        if ((ID_par.eq.5).or.(ID_par.eq.6).or.(ID_par.lt.0)
     &                 .or.(ID_par.eq.75).or.(ID_par.eq.76)) then
          if (ID_par.eq.75 .or. ID_par.eq.76) then
            write(6,'(/,a)') 'muon production point entry found:'
          else if (ID_par.lt.0) then
            write(6,'(a)') 'muon mother/grandmother entry found:'
          else if (ID_par.eq.5 .or. ID_par.eq.6) then
            write(6,'(a)') 'muon entry found:'
          endif
          if ((ID_par.ge.70).or.(ID_par.lt.0)) then
            hh = CURPAR(7)
            write(6,'(i4,1p,4e11.3,3e13.5)')ID_par,Elab,px,py,pz,
     *                                                  xx,yy,hh
          else
            write(6,'(i4,1p,4e11.3,2e13.5)') ID_par,Elab,px,py,pz,xx,yy
          endif
        endif
