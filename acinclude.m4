...
[
  ...
  if test "$(basename $F77)" = "gfortran" ; then
    ...
    if test $gfortran_version1 -ge 4 && test $gfortran_version2 -ge 2 ;then
       USE_GFORTRANEW=true
       FFLAGS="$FFLAGS -Wtabs"
    fi    
    USE_GFORTRAN=true
  ...
  USE_IFORT=false
  if test "$(basename $F77)" = "ifort" ; then
  ...
  NOTUSE32=false
  case $host in
       *86*64*linux* ) if test "$(basename $F77)" = "g77" ; then
       ...
       *86*linux*    ) if test "$(basename $F77)" = "g77" ; then
                          FFLAGS="$FFLAGS -fbounds-check"
                       fi
                       CORSIKA_FFLAGS="-O0"
                       BERNOS="Linux" 
       ...
       *darwin*	     ) if test "$(basename $F77)" = "g77" ; then
                          FFLAGS="$FFLAGS -fbounds-check"
                       fi
                       CORSIKA_FFLAGS="-O0"
                       BERNOS="mac" 
  ...
  # ------ trapfe -------
  AM_CONDITIONAL([USE_TRAPFE], test "x$(basename $F77)" = "xg77" && test "x$BERNOS" = "xLinux" )
  
  # ------ urqmd -------
  case $USEROS in
    AIX    ) URQMD_FFLAGS='-O5 -qextname -qstrict -qipa=partition=large' ;;
    Linux  ) URQMD_FFLAGS='-O3 -Wall -Wsurprising' ;;
    Intel  ) URQMD_FFLAGS='-O3 -axMK -tpp6 -w95 -ipo' ;;
  ...
  if $USE_GFORTRAN ; then
    ...
    case $USEROS in
      ...
      OSF1   ) DPM_FFLAGS="-C -fpe4 -check_underflow -check_overflow -g3 -O1" ;;
      *      ) DPM_FFLAGS="" ;;
    esac
  fi
...
[
  ...
  case $CORHEMODEL in
      ...
      SIBYLL    )
                 ...
                 AC_DEFINE([__SIBYLL__],[1],[include Sybill 2.1 program])
...
[_CORSIKA_ENABLE_OPTION([CERENKOV], 
                        [VOLUMECORR INTTEST ANAHIST AUGERHIST MUONHIST AUGCERLONG COMPACT], [],
...
[_CORSIKA_ENABLE_OPTION([INTTEST], 
                        [MACM68 CERENKOV IACT CEFFIC ATMEXT THIN FLUKA NEUTRINO PLOTSH PLOTSH2 CURVED UPWARD VIEWCONE ANAHIST AUGERHIST MUONHIST AUGCERLONG PRESHOWER COMPACT HERWIG STACKIN CONEX], [],
...
[_CORSIKA_ENABLE_OPTION([COMPACT], 
                        [CERENKOV IACT INTTEST ROOTOUT MACHINEINDEPENDENT], [], 
...
[_CORSIKA_ENABLE_OPTION([PARALLEL], [ANAHIST AUGERHIST MUONHIST CERENKOV IACT COMPACT INTTEST MAC PLOTSH PLOTSH2 ROOTOUT CEFFIC], [], [no], [parallel computation], [__PARALLEL__], [ 
...
[_CORSIKA_ENABLE_OPTION([CHARM], [EPOS NEXUS QGSJETII SIBYLL VENUS], [], [no] ,[pythia charm production], [__CHARM__], 
    if test "x$CORHEMODEL" != "xDPMJET" ; then
      [CORSIKA_REQUIRE_PYTHIA]
    fi)])

AC_DEFUN([CORSIKA_ENABLE_CONEX],
[_CORSIKA_ENABLE_OPTION([CONEX], [DPMJET VENUS INTTEST CERENKOV], [THIN CURVED SLANT UPWARD], [no] ,[CONEX cascade equation (CONEX)], [__CONEX__], 
    [
...
[_CORSIKA_WITH_PACKAGE([cernlib],
  ...
  [$CERN_ROOT $CERNLIB /cern/pro],
  [lib/libkernlib.a])
if test x$corsika_cv_root_cernlib != xno
then
   CERNLIB_FFLAGS= 
   CERNLIB_LIBS="-L$corsika_cv_root_cernlib/lib -lmathlib -lpacklib -lkernlib"
   _CORSIKA_SAVE_COMPILE_FLAGS
...
# __COAST__ defined CORSIKA_REQUIRE_COAST only for CORSIKA, do it here for CONEX
    if test x$COAST_INTERFACE_LIBS != x ; then 
...
[AC_ARG_ENABLE([debug],
               AC_HELP_STRING([--enable-debug],
                              [enables debug symbols (default=no)]),
               [corsika_cv_enable_debug=$enableval],
               [corsika_cv_enable_debug='no'])
