...
bool GroundArea::WriteResults(string parShowerName, string parSimName_coreas)
{
  errno = 0;
  int result = mkdir(parShowerName.c_str(), 0777);
  if ( (result == 0) || (errno == EEXIST) )
  {
    ofstream bout( (parShowerName+".bins").c_str() ); // open file to list the active bins
    bool allok = bout;                                // opening ok?
    for (vector<GroundElement*>::const_iterator i = itsActiveGroundElementPointers.begin(); i != itsActiveGroundElementPointers.end(); ++i)
    {
      stringstream ss;
      //use relative path
      ss << parSimName_coreas << "/raw_" << (*i)->GetObserverName() << ".dat";
      ...
      stringstream rawss;
      rawss << parShowerName << "/raw_" << (*i)->GetObserverName() << ".dat";
