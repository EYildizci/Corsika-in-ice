...
ScenarioParams::ScenarioParams(const string& parName, const string& parGroundFileName)
: itsName(parName),
...
{
  // export the scenario parameters
  bool FirstOk;
  ofstream fout((itsName+".reas").c_str());
  fout << *this;
  FirstOk = fout;
  fout.close();
...
istream& operator>> (istream& is, ScenarioParams& rhs)
{
   ...
   while ( is.getline(lineBuf,1024) )
   {
    ...
    if ( pos < currLine.size() )
    {
      ...
      if (type.find("GPSNanoSecs") < tend) { ss >> ll; rhs.itsGPSNanoSecs=ll; continue; }
