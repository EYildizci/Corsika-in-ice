C***********************************************************************
      SUBROUTINE PLOMBR(POD,IJPROJ,IFL,PPD,CHARD,XFD,XLAD,PTD,
     *PPCMSD,YLABD,ETALABD,ETACMD,YCMD,YBEAMD, 
     *ITIF,THETAD,PMODD,IBARF)                 
...
*...CHECK OF X AND Q2 VALUES :
      IF ( (X.LT.0.99D-9) .OR. (X.GT.1.D0) ) THEN
         INIWA1=INIWA1+1
         IF(INIWA1.LE.-10)WRITE(6,91)
  91     FORMAT (2X,'grv98lo PARTON INTERPOLATION: X OUT OF RANGE',
     *     ' Only 10 warnings')
C                              j.r.11/98  
C         STOP
      ENDIF
      IF ( (Q2.LT.0.799) .OR. (Q2.GT.1.E6) ) THEN
         INIWA2=INIWA2+1
         IF(INIWA2.LE.-10)WRITE(6,92)
  92     FORMAT (2X,'grv98lo PARTON INTERPOLATION: Q2 OUT OF RANGE',
     *     ' Only 10 warnings')
C                              j.r.11/98  
C        STOP
      ENDIF
...
*...INTERPOLATION :
      ...
      XV = X**0.5
      XS = X**(-0.2)
      UV = PO_FINT(NARG,XT,NA,ARRF,XUVF) * X1**3 * XV
      DV = PO_FINT(NARG,XT,NA,ARRF,XDVF) * X1**4 * XV
      DE = PO_FINT(NARG,XT,NA,ARRF,XDEF) * X1**7 * XV
      UD = PO_FINT(NARG,XT,NA,ARRF,XUDF) * X1**7 * XS
      US = 0.5 * (UD - DE)
      DS = 0.5 * (UD + DE)
      SS = PO_FINT(NARG,XT,NA,ARRF,XSF)  * X1**7 * XS
      GL = PO_FINT(NARG,XT,NA,ARRF,XGF)  * X1**5 * XS

      END


      FUNCTION PO_FINT(NARG,ARG,NENT,ENT,TABLE)
C***********************************************************************
C
C     CERN library E104
C
C     multi-dimensional interpolation routine, needed for GRV98 PDFs
C
